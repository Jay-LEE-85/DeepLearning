...
이러한 문제를 해결하고자 고안된 기법이 바로 **드롭아웃**입니다. 드롭우웃은 신경망의 각층의 노드를 훈련과정에서 임의로 삭제하면서 신호전달을 차단하는 방법입니다. 다만, 시험 또는 검증과정에서는 모든 노드를 사용합니다.^[주의 할 것은 검증 과정에서 각 노드 또는 뉴런의 출력에 훈련때 삭제 안한 비율을 곱하여 출력을 수행합니다.]

![Concept of Dropout](image/fig-techOver1.png){#fig-techOver1}

위에서 설명한 드롭아웃을 구현하도록 하겠습니다. 먼저 순전파를 담당하는 `forward`메서드에서는 훈련 때(`train_flg=True`)만 잘 계산해두면 검증 때 단순히 데이터를 흘리기만 하면됩니다.^[삭제 안한 비율은 곱하지 않아도 됩니다.]
...
드롭아웃을 적용한 경우 훈련 데이터와 검증 데이터간 정확도의 차이가 확연하게 줄어 들었음을 확인할 수 있습니다. 동시에 표현력도 상당히 개선되었음을 확인할 수 있습니다.
