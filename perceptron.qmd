...
노란색 노드에서는 2단계 계산이 발생합니다. 하나늗 입력값의 뉴런에서는 신호의 세기를 측정하는 **Weighted Sum**과 

$$
z = b + w_{1}x_{1} + w_{2}x_{2} + \cdots + w_{n}x_{n} = b + \textbf{w}^{T} \textbf{x}
$$ {#eq-weightedSum}

뉴런의 여부를 계산하는 **Step Function**으로 구성되어 있습니다.
...
::: {.callout-note}
## 퍼셉트론에서 사용되는 계단함수
일반적으로 **Sign function**을 가장 많이 사용하지만, 이진값(0, 1)만을 갖는 **Heavisde step function**이 사용되기도 합니다.
$$
